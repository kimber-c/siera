        ...
        <div class="col-lg-12">
            <div class="row">
                <div class="col-lg-6 col-sm-6 col-12 m-auto">
                    <h3><i class="fa fa-building"></i> Evaluaciones</h3>
                </div>
                <div class="col-lg-6 col-sm-6 col-12 m-auto">
                        ...
                        <div class="col-md-12 table-responsive contenedorRegistros p-0" style="display: none;">
                            <table id="registros" class="table table-hover dt-responsive nowrap">
                                <thead class="thead-light">
                                    <tr>
                                        <th class="text-center" data-priority="2">Año</th>
                                        <th class="text-center" data-priority="2">Etapa</th>
...
<?php include 'modals/modal_evaluacion.php';?>
<script>
...
function fillRegistros()
{
    ...
    jQuery.ajax(
    { 
        url: "<?php echo base_url('evaluacion/listar');?>",
        method: 'get',
        success: function(r){
            var html = '';
            ...
            for (var i = 0; i < data.length; i++) 
            {
                html += '<tr class="text-center">' +
                    '<td>' + novDato(data[i].anio) + '</td>' +
                    '<td>' + novDato(data[i].etapa) + '</td>' +
                    '<td>'+
                    '<div class="btn-group btn-group-sm" role="group">'+
                        '<button type="button" class="btn text-secondary" title="Editar registro" onclick="consultar(this);" data-id="'+data[i].idevaluacion+'"><i class="fa fa-edit" ></i></button>'+
                        '<button type="button" class="btn text-danger" title="Eliminar registro" onclick="eliminar(this);" data-id="'+data[i].idevaluacion+'"><i class="fa fa-trash"></i></button>'+
                    '</div>'+
                    '</td></tr>';
            }
            $('#data').html(html);
...
function eliminar(elem)
{
    Swal.fire({
        ...
        if(result.isConfirmed)
        {
            ...
            jQuery.ajax(
            { 
                url: "<?php echo base_url('evaluacion/eliminar');?>",
